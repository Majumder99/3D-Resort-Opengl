...
//eikhane tree
		...
		for (int i = 0; i < 100; i++) {
			...
			else if (i <= 25)
			{
				...
				translateMatrix = glm::translate(identityMatrix, glm::vec3(8.72, 1.939, 5.0));
				color1 = glm::vec4(1.0f, 1.0f, 1.0f, 1.0f);
				dotdot(lightingShader, translateMatrix, color1, all_cubes);

			}
			else {
				translateMatrix = glm::translate(identityMatrix, glm::vec3(8.72, 2.3 - dy + globalDotStep, 5.0));
				color1 = glm::vec4(1.0f, 1.0f, 1.0f, 1.0f);
				dotdot(lightingShader, translateMatrix, color1, all_cubes);
			}
